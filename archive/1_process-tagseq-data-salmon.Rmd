---
title: "Process tagseq data - Berdahl salmon samples [DEPRECATED - see tag-seq/code/01-upstream-alignment.qmd]"
output: html_notebook
---

**DEPRECATED:** This file is retained for historical reference only. Use `tag-seq/code/01-upstream-alignment.qmd` 
for the current, cleaned-up version with all absolute paths parameterized, bowtie2 removed, and brain 
branch clearly marked as unresolved.

It lives in `archive/` and is excluded from the Quarto project's `render:` list, so it is never
rendered as part of the analysis. The guard below is the second line of defence: every chunk below
is a shell transcript of one session on one machine — absolute paths under `/home/shared/`, `wget`
against Gannet, `rsync` between local directories — and none of it should execute anywhere.

```{r deprecated-guard, include=FALSE}
