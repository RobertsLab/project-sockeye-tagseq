...
## On a fresh clone, run renv::restore() once, then render. Do not install
## packages from inside this notebook: installing at render time means the
## result depends on whatever CRAN served that day.

## Every package below is called by this notebook. Thirteen that were loaded
## here and never called have been dropped (edgeR, goseq, GenomicFeatures,
## data.table, calibrate, affycoretools, vsn, tidybulk, cowplot, gplots,
## plotly, readxl, sqldf), along with EnhancedVolcano and pcaExplorer, whose
## functions are never invoked -- the volcano plots are plain ggplot2.

## library() is used rather than require(): a missing package must stop the
## render here, not return FALSE and fail later inside an analysis chunk.
...
trt_list <- read.csv("data/treatments-liver.csv", sep=',', header=TRUE, row.names = "ID")
coldata <- trt_list

# Import gene_count_matrix
cts <- as.matrix(read.csv("data/onerka_gene_count_matrix-liver.csv", sep=',', header=TRUE, row.names = "gene_id"))
...
setwd("DESEQ_output/liver")

ggexport(filename = "liver-PCA.png",
         plot   = bp1,
         res    = 600,
         # device = "png",
         width  = 4000,
...
ggexport(filename = "liver-PAIRS.png",
         plot   = bp2,
         res    = 600,
         width  = 6000,
         height = 6000)

ggexport(filename = "liver-pheatmap.png",
         plot   = bp3,
         res    = 600,
         width  = 6000,
         height = 6000)
...
pdf(file= "DESEQ_output/liver/MA_plots.pdf" )
par(mfrow=c(1,3), mar=c(4,4,2,1))
xlim <- c(1,1e5); ylim <- c(-3,3)
DESeq2::plotMA(res_table_normal, xlim=xlim, ylim=ylim, main="normal", cex=.8)
abline(h=c(-1.5,1.5), col="dodgerblue", lwd=2)
...
```{r}
# nrow(res_table)
print(paste("Total genes:", nrow(res_table)))
print(paste("Genes after NAs removed:", nrow(na.omit(res_table))))
...
all_genes_normal$treatment <- "liver"
...
sig_genes_normal$treatment <- "liver" 
...
sig_genes_apeglm$treatment <- "liver" 
...
sig_genes_ashr$treatment <- "liver" 
...
write.table(single_gene_expression_unshrunk, file = "DESEQ_output/liver/liver-unshrunk-single-gene-counts.csv", row.names = FALSE)
write.table(single_gene_expression_normal, file = "DESEQ_output/liver/liver-normal-single-gene-counts.csv", row.names = FALSE)
write.table(single_gene_expression_apeglm, file = "DESEQ_output/liver/liver-apeglm-single-gene-counts.csv", row.names = FALSE)
write.table(single_gene_expression_ashr, file = "DESEQ_output/liver/liver-ashr-single-gene-counts.csv", row.names = FALSE)
...
write.table(all_genes,        file = "DESEQ_output/liver/liver-ALL-DEG.csv",            row.names = FALSE, sep = ",")
write.table(all_genes_normal, file = "DESEQ_output/liver/liver-ALL-DEG-normal.csv",     row.names = FALSE, sep = ",")
write.table(all_genes_apeglm, file = "DESEQ_output/liver/liver-ALL-DEG-apeglm.csv",     row.names = FALSE, sep = ",")
write.table(all_genes_ashr,   file = "DESEQ_output/liver/liver-ALL-DEG-ashr.csv",       row.names = FALSE, sep = ",")
write.table(sig_genes,        file = "DESEQ_output/liver/liver-SIG-DEG-unshrunken.csv", row.names = FALSE, sep = ",")
write.table(sig_genes_normal, file = "DESEQ_output/liver/liver-SIG-DEG-normal.csv",     row.names = FALSE, sep = ",")
write.table(sig_genes_apeglm, file = "DESEQ_output/liver/liver-SIG-DEG-apeglm.csv",     row.names = FALSE, sep = ",")
write.table(sig_genes_ashr,   file = "DESEQ_output/liver/liver-SIG-DEG-ashr.csv",       row.names = FALSE, sep = ",")
write.table(gene_counts,      file = "DESEQ_output/liver/liver-gene-counts.csv",        row.names = TRUE)
...
p8 <- ggplot(sig_genes_apeglm, aes(x = log2FoldChange, y = -log10(pvalue))) +
             geom_point(aes(color=log2FoldChange>0))
p8
ggsave("DESEQ_output/liver/Volcano_all_genes.png",
       plot   = p1,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_all_genes_normal.png",
       plot   = p2,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_all_genes_ashr.png",
       plot   = p3,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_all_genes_apeglm.png",
       plot   = p4,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_sig_genes.png",
       plot   = p5,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_sig_genes_normal.png",
       plot   = p6,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_sig_genes_ashr.png",
       plot   = p7,
       dpi    = 600,
       device = "png",
       width  = 6,
...
ggsave("DESEQ_output/liver/Volcano_sig_genes_apeglm.png",
       plot   = p8,
       dpi    = 600,
       device = "png",
       width  = 6,
...
# Plot heatmap
png(filename = "DESEQ_output/liver/liver_heatmap_pval_genes_apeglm.png", 
...
heatmap(mat.scaled_sigs, scale = "none", col =  col, 
        ColSideColors = c(rep("royalblue1", 15), rep("red3", 15)))
dev.off()
```
